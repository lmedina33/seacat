...
db = DAL(DBURI,
         ...
         #migrate_enabled=False  ## Comment this line if your DB doesn't exists.
         )
...
db.define_table(auth.settings.table_user_name,
                ...
                Field('email', 'string', length=128, requires=[IS_NOT_IN_DB(db, 'auth_user.email'), IS_EMAIL()], label=T("Email")),
                Field('password', 'password', length=512, readable=False, requires=CRYPT(), label=T('Password')),
                Field('gender', 'string', length=1, requires=IS_IN_SET(GENDER_LIST, zero=None), label=T("Gender")),
...
db.define_table('address',
                ...
                Field('floor', 'string', length=3, default="", label=T("Floor")),
                #Field('door', 'string', length=3, default="", label=T("Door")),
                Field('apartment', 'string', length=3, default="", label=T("Apartment")),
                Field('street1', 'string', length=128, default="", label=T("Between Street 1"), comment=T("(optional)")),
                Field('street2', 'string', length=128, default="", label=T("Between Street 2"), comment=T("(optional)")),
                Field('zip_code', 'string', length=10, default="", label=T("ZIP Code")),
                ...
                Field('obs', 'text', default="", label=T("Observations")),
                auth.signature,
...
db.define_table('personal_data',
                ...
                Field('cuil', 'string', length=11, requires=IS_EMPTY_OR(IS_MATCH('\d{11}', error_message=T("Please verify CUIL format, it must have 11 digits!"))), label="CUIL", comment=T("Insert only numbers")+" i.e.: 20126548971"),
                ...
                Field('tel1', length=10, requires=IS_MATCH('^\d{6,10}$'), label=T("Principal Phone Number"), comment=T("Insert only numbers, without area code and dashes.")+" i.e.: 49811337"),
                ...
                Field('tel2', length=10, requires=IS_EMPTY_OR(IS_MATCH('^\d{6,10}$')), label=T("Alternative Phone Number"), comment=T("Insert only numbers, without area code and dashes.")+" i.e.: 49811985"),
...
db.define_table('parent',
                ...
                Field('children_course', 'string', length=16, requires=IS_EMPTY_OR(IS_IN_SET((COURSE[0], COURSE[1], COURSE[2]))), label=T("Children course")),
                ...
                Field('accepted_terms', 'datetime', default=None, label=T("Accepted Terms & Conditions on")),
                auth.signature,
                format='%(db.auth_user.last_name)s'+', '+'%(db.auth_user.first_name)s'+' '+'%(db.auth_user.middle_name)s'
               )
